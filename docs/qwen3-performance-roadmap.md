...
- 针对 `prompt <= 2k` 的主场景，scheduler 已加入 decode-first admission burst：当 waiting queue 增长、prefill 晋升为 running、或 running batch 缩小时，会优先保住几轮稳定 decode；当前策略是 `4+ lane` 批次才会因 queue pressure / prefill 晋升而延迟 admission，而 `4 -> 3` shrink 只保留 1 轮 decode，随后尽快补回第 4 条。
...
- scheduler 已进一步加入短 prompt 吞吐优先的 decode-burst admission 策略，覆盖 `新请求入队 / prefill 完成 / batch shrink` 三类事件；当前 tensor-KV 路径下，这比“slot 一空就立刻 prefill”更符合 Qwen3 1.7B serving 的真实瓶颈。当前实现是：`4+ lane` 稳定批次优先保批，`4 -> 3` shrink 只保留 1 轮 decode，然后尽快补位，避免长期停留在 3-lane。
